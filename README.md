...
```
   Orchestrator (Pydantic AI + OpenAI) — plans & calls tools, STREAMS its trace
         │                                                  │
   Structured tools (deterministic)                  Retrieval tool (hybrid)
   course_details / prerequisites / check_eligibility      search_handbook(query)
   courses_unlocked_by / course_offerings / is_offered      embeddings + keyword
   list_programs / program_requirements / degree_progress    over policy/advising prose
         │
...
- **Prose** — policies / student life / advising docs: OpenAI embeddings + hybrid
  search. Not graphed.
...
src/advisor/
  ...
  retrieval/           # chunk prose, OpenAI embeddings, hybrid search
...
uv run advisor index                 # build the prose embedding index (one-time)
...
uv run pytest                        # 30 tests (incl. 100% prereq parse on real data)
```
